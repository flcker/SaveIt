...
function App() {
  ...
  // Track active tab in current window
  ...
  onMount(() => {
    ...
    api.tabs.onActivated.addListener(() => updateActiveTab());
    api.tabs.onUpdated.addListener((_id, info) => {
      if (info.title) updateActiveTab();
    });
  });
  ...
  return (
    <div>
      <header style={{ "margin-bottom": "8px", display: "flex", "align-items": "center", "justify-content": "space-between" }}>
        <h1 style={{ "font-size": "16px", "font-weight": "600" }}>SaveIt</h1>
      </header>
      ...
      <div style={{ display: "flex", gap: "4px", "margin-bottom": "12px" }}>
        <TabButton active={mode() === 'page'} onClick={() => { setMode('page'); setStatus('idle'); setMessage(''); }} label="保存页面" />
        <TabButton active={mode() === 'site'} onClick={() => { setMode('site'); setStatus('idle'); setMessage(''); }} label="保存站点" />
      </div>

      <Show when={mode() === 'page'}>
        <button onClick={handleSavePage} disabled={status() === 'capturing'} style={btnStyle(status() === 'capturing')}>
      ...
      <Show when={mode() === 'site'}>
        <Show when={status() === 'idle' || status() === 'error'}>
          <button onClick={handleDiscover} style={btnStyle(false)}>发现页面结构</button>
        </Show>

        <Show when={status() === 'discovering'}>
          <div style={{ padding: "12px", "text-align": "center", color: "#666" }}>正在分析页面导航...</div>
        </Show>

        <Show when={status() === 'previewing'}>
          <div style={{ "max-height": "calc(100vh - 280px)", overflow: "auto", border: "1px solid #e2e8f0", "border-radius": "6px", padding: "8px", "margin-bottom": "8px" }}>
        ...
        <Show when={status() === 'crawling'}>
          <ProgressBar completed={progress().completed} total={progress().total} currentUrl={progress().currentUrl} />
        </Show>
      </Show>
      ...
      <Show when={debugLogs().length > 0}>
        <details style={{ "margin-top": "8px", "font-size": "11px" }}>
          <summary style={{ cursor: "pointer", color: "#64748b" }}>调试信息</summary>
          <div style={{ "margin-top": "4px", background: "#f1f5f9", "border-radius": "4px", padding: "8px" }}>
            <For each={debugLogs()}>
              {(log) => (
                <div style={{ "margin-bottom": "4px", "font-family": "monospace" }}>
                  <span style={{ color: log.detected ? '#16a34a' : '#94a3b8' }}>{log.detected ? '✓' : '✗'}</span>
...
function TabButton(props: { active: boolean; onClick: () => void; label: string }) {
  return (
    <button onClick={props.onClick} style={{
      ...
      "font-size": "13px", "font-weight": props.active ? "600" : "400", cursor: "pointer",
    }}>
      {props.label}
    </button>
  );
        ...
        {(node) => (
          <li style={{ "padding-left": `${node.depth * 16}px` }}>
            <label style={{ display: "flex", "align-items": "center", gap: "4px", padding: "2px 0", cursor: "pointer" }}>
              <input type="checkbox" checked={node.selected} onChange={() => props.onToggle(node)} />
              <span style={{ overflow: "hidden", "text-overflow": "ellipsis", "white-space": "nowrap" }}>{node.title || node.url}</span>
            </label>
  ...
  return (
    <div style={{ padding: "8px 0" }}>
      <div style={{ display: "flex", "justify-content": "space-between", "font-size": "12px", color: "#666", "margin-bottom": "4px" }}>
        <span>{props.completed} / {props.total}</span><span>{pct()}%</span>
      </div>
      <div style={{ height: "4px", background: "#e2e8f0", "border-radius": "2px", overflow: "hidden" }}>
        <div style={{ height: "100%", background: "#2563eb", width: `${pct()}%`, transition: "width 0.3s" }} />
      </div>
      <div style={{ "font-size": "11px", color: "#999", "margin-top": "4px", overflow: "hidden", "text-overflow": "ellipsis", "white-space": "nowrap" }}>{props.currentUrl}</div>
    </div>
  );
}
...
function btnStyle(disabled: boolean) {
  return {
    width: "100%", padding: "10px 16px", "border-radius": "6px", border: "none",
    background: disabled ? '#94a3b8' : '#2563eb', color: "#fff",
    "font-size": "14px", "font-weight": "500", cursor: disabled ? 'wait' : 'pointer',
  };
}

function countNodes(nodes: NavNode[]): number {
  let count = 0;
  for (const node of nodes) { if (node.url) count++; count += countNodes(node.children); }
  return count;
}

function getSelectedUrls(nodes: NavNode[]): string[] {
  const urls: string[] = [];
  for (const node of nodes) { if (node.selected && node.url) urls.push(node.url); urls.push(...getSelectedUrls(node.children)); }
  return urls;
}

function toggleChildren(node: NavNode, selected: boolean) {
  for (const child of node.children) { child.selected = selected; toggleChildren(child, selected); }
}
